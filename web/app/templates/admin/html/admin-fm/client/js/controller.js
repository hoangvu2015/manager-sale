angular.module('fm').controller('FileManagerCtr', ['$scope', '$http', '$location', '$state',
    function($scope, $http, $location, $state) {
        var FM = this;
        FM.fileUrl = window.settings.services.webUrl + '/files/'
        ...
        FM.onLoad = function(instance) {
            instance.expandAll();
        };

        FM.changeOptions = function() {
            FM.file.options.mode = FM.file.options.mode == 'tree' ? 'code' : 'tree';
        };
        ...
        FM.update = function() {
            ...
            var req = {
                method: 'POST',
                url: settings.services.apiUrl + '/fm/edit',
                data: { fileName: fileName, data: file.data }
            }
            $http(req)
                .then(function(res) {
        ...
        FM.findOne = function() {
            ...
            $http(req)
                .then(function(res) {
                    FM.file = { data: JSON.parse(res.data.file) };
                })
                .catch(function(err) {
                    alert('Error: ' + err);
                });
        ...
        var setCurFiles = function(relPath) {
            var req = {
                method: 'POST',
                url: settings.services.apiUrl + '/fm',
                headers: {
                    ...
                    if (!angular.equals(res.data, {})) {
                        ...
                        files.forEach(function(file) {
                            ...
                            if ($state.params.path != "") {
                                file.relPath = $state.params.path + '/' + file.name;
                            }
                            file.selected = false;
        ...
        var httpRequest = function(method, url, params, data, config) {
            ...
            $http(conf)
                .then(function(data) {
                    if (data.data.status) {
                        FM.successData = data.data.message;
        ...
        FM.delete = function() {
            console.log(FM.curFiles, "curFiles");

            for (var i in FM.selection) {
                var relPath = FM.selection[i].relPath;
                var url = settings.services.apiUrl + '/fm/delete';
